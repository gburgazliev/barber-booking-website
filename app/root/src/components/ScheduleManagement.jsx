...
const ScheduleManagement = () => {
  ...
  const fetchWorkingHours = async () => {
    try {
      setLoading(true);
      const response = await fetch(SERVER_URL(`api/schedule/get-working-hours/:${formattedDate}`));
      ...
      addAlert(`Error loading working hours: ${error.message}`, ALERT_TYPES.ERROR);
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchAppointments = async () => {
    try {
      const response = await fetch(SERVER_URL(`api/appointments/:${formattedDate}`));
      ...
      if (response.ok) {
        ...
        const updatedSlots = timeSlots.map(slot => {
          const isBooked = data.appointments.some(
            appointment => appointment.timeSlot === slot.time && appointment.status === 'Confirmed'
          );
  ...
  const handleSaveWorkingHours = async () => {
    try {
      ...
      addAlert(`Error updating working hours: ${error.message}`, ALERT_TYPES.ERROR);
    } finally {
      setSaving(false);
    }
  };
      ...
      <div className="card bg-base-100 shadow-xl mt-8">
        <div className="card-body">
          ...
          <div className="alert alert-info mb-4">
            <span>You can set special working hours for holidays or other events. These will override the regular schedule.</span>
          </div>
